...
Sensor_Array::Sensor_Array(uint8_t one_wire_pin, uint8_t dht_pin1, uint8_t dht_pin2, uint8_t dht_type) 
    : _dht{
        DHT_Unified(dht_pin1, dht_type, 6, 1, 2),
        DHT_Unified(dht_pin2, dht_type, 6, 3, 4)
    },
    _dsx(one_wire_pin),
    _sensors{nullptr},
    _sensor_cnt(0),
...
bool Sensor_Array::isValidIndex(uint8_t index) const
{
    return index < _sensor_cnt && _sensors[index];
}

bool Sensor_Array::getEvent(uint8_t index, sensors_event_t* event)
{
    if (!isValidIndex(index)) return false;
    return _sensors[index]->getEvent(event);
}

bool Sensor_Array::getSensor(uint8_t index, sensor_t* sensor)
{
    if (!isValidIndex(index)) return false;
